...
export class BlenderCollection<T> {
  constructor(private interop: BlenderInterop, private accessor: string, private _constructor: Constructor<T>) {}
  ...
  public get(key: string): T {
    const command = isNaN(key as any) ? `${this.accessor}.get("${key}")` : `${this.accessor}[${key}]`
    const ref = this.interop.execute(`${command}`)
    return new this._constructor(this.interop, ref)
  }
  ...
  public values(): T[] {
    ...
    if (keys.length === 0) {
      ...
      return array.map((accessor: string) => new this._constructor(this.interop, accessor))
    } else {
  ...
  public static createGeneric<T>(
    interop: BlenderInterop,
    accessor: string,
    item_constructor: Constructor<T>
  ): BlenderCollection<T> & Indexable<T> {
  ...
  public static createSpecialized<S, T>(
    interop: BlenderInterop,
    accessor: string,
    specialized_constructor: Constructor<S>,
    item_constructor: Constructor<T>
  ): BlenderCollection<T> & Indexable<T> & S {

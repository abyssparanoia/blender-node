...
export class PoseOperators {
   ...
   * @desc void
   */
  public breakdown(options: {
    percentage?: number
    prev_frame?: number
    next_frame?: number
    channels?: 'ALL' | 'LOC' | 'ROT' | 'SIZE' | 'BBONE' | 'CUSTOM'
    axis_lock?: 'FREE' | 'X' | 'Y' | 'Z'
  }): void {
   ...
   * @desc void
   */
  public constraint_add_with_targets(options: {
    type?:
      | 'CAMERA_SOLVER'
      | 'FOLLOW_TRACK'
      | 'OBJECT_SOLVER'
   ...
   * @desc void
   */
  public paths_calculate(options: {
    start_frame?: number
    end_frame?: number
    bake_location?: 'HEADS' | 'TAILS'
  }): void {
   ...
   * @desc void
   */
  public propagate(options: {
    mode?: 'WHILE_HELD' | 'NEXT_KEY' | 'LAST_KEY' | 'BEFORE_FRAME' | 'BEFORE_END' | 'SELECTED_KEYS' | 'SELECTED_MARKERS'
    end_frame?: number
  }): void {
   ...
   * @desc void
   */
  public rotation_mode_set(options: {
    type?: 'QUATERNION' | 'XYZ' | 'XZY' | 'YXZ' | 'YZX' | 'ZXY' | 'ZYX' | 'AXIS_ANGLE'
  }): void {

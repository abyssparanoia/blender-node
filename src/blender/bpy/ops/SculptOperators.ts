...
export class SculptOperators {
   ...
   * @desc void
   */
  public brush_stroke(options: {
    stroke?: string
    mode?: 'NORMAL' | 'INVERT' | 'SMOOTH'
    ignore_background_click?: boolean
  }): void {
   ...
   * @desc void
   */
  public mask_expand(options: {
    invert?: boolean
    use_cursor?: boolean
    update_pivot?: boolean
    smooth_iterations?: number
    mask_speed?: number
    use_normals?: boolean
    keep_previous_mask?: boolean
   ...
   * @desc void
   */
  public mask_filter(options: {
    filter_type?: 'SMOOTH' | 'SHARPEN' | 'GROW' | 'SHRINK' | 'CONTRAST_INCREASE' | 'CONTRAST_DECREASE'
    iterations?: number
    auto_iteration_count?: boolean
  }): void {
   ...
   * @desc void
   */
  public mesh_filter(options: {
    type?: 'SMOOTH' | 'SCALE' | 'INFLATE' | 'SPHERE' | 'RANDOM' | 'RELAX'
    strength?: number
    deform_axis?: ('X' | 'Y' | 'Z')[]
  }): void {

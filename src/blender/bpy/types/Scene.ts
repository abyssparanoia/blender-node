...
export class Scene {
  ...
  public get sequencer_colorspace_settings(): ColorManagedSequencerColorspaceSettings {
    return PythonInterop.getClass(
      this.interop,
      `${this.accessor}.sequencer_colorspace_settings`,
  ...
  public get timeline_markers(): BlenderCollection<TimelineMarker> & Indexable<TimelineMarker> & TimelineMarkers {
    return BlenderCollection.createSpecialized(
      this.interop,
      `${this.accessor}.timeline_markers`,
      TimelineMarkers,
      TimelineMarker
    )
  }
   ...
   */
  public get transform_orientation_slots(): BlenderCollection<TransformOrientationSlot> &
    Indexable<TransformOrientationSlot> {
    return BlenderCollection.createGeneric(
      this.interop,
      `${this.accessor}.transform_orientation_slots`,
   ...
   */
  public get audio_distance_model():
    | 'NONE'
    | 'INVERSE'
    | 'INVERSE_CLAMPED'
    | 'LINEAR'
   ...
   * @desc enum in ['NONE', 'INVERSE', 'INVERSE_CLAMPED', 'LINEAR', 'LINEAR_CLAMPED', 'EXPONENT', 'EXPONENT_CLAMPED'], default 'NONE'
   */
  public set audio_distance_model(
    value: 'NONE' | 'INVERSE' | 'INVERSE_CLAMPED' | 'LINEAR' | 'LINEAR_CLAMPED' | 'EXPONENT' | 'EXPONENT_CLAMPED'
  ) {
